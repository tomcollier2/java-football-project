...
public class FootieDashboard {
	...
	/**
	 * Main method - receives two CSV files with the relevant player entries data
	 * and a text file
	...
	public static void main(String[] args) {
		...
		FootieDashboard footieDashboard = new FootieDashboard(eplFile, ligaFile, queriesFile);
	...
	/**
	 * Display the number of unique players for whole dataset, and number of player
	 * entries
	...
	private void printPlayerEntries(Collection<PlayerEntry> playerEntries) {
		...
		for (PlayerEntry w : playerEntries) {
			...
			i++;
			// ensures only first 5 players are shown
			if (i > 4)
				return;
		}
	}
	...
	/**
	 * this method pronts the first 5 players in the prem and liga to the terminal
	 */
